// elm-watch hot {"version":"1.0.2","webSocketPort":38745}
"use strict";
(() => {
...
var $author$project$Main$Recording = F3(
	function (videoId, time, text) {
		return {text: text, time: time, videoId: videoId};
	});
...
var $author$project$Main$decodeRecordings = $elm$json$Json$Decode$list(
	A4(
...
var $author$project$Main$backendUrlRoot = 'https://raw.githubusercontent.com/bryanjenningz/listening-practice-web-app/main/static/';
var $author$project$Video$Video = F4(
	function (id, title, duration, subtitles) {
		return {duration: duration, id: id, subtitles: subtitles, title: title};
	});
...
var $author$project$Video$Subtitle = F3(
	function (videoId, text, time) {
		return {text: text, time: time, videoId: videoId};
	});
...
var $author$project$Video$parseInt = A2(
...
var $author$project$Video$parseTime = A2(
	$elm$parser$Parser$keeper,
	A2(
		$elm$parser$Parser$keeper,
		A2(
		...
		_List_fromArray(
			[
				...
				A2(
					...
					$elm$parser$Parser$succeed($elm$core$Basics$identity),
					$elm$parser$Parser$symbol(':')),
				A2($elm$parser$Parser$map, $elm$core$Maybe$Just, $author$project$Video$parseInt)),
...
var $author$project$Video$fromStrTime = function (strTime) {
	return $elm$core$Result$toMaybe(
		A2($elm$parser$Parser$run, $author$project$Video$parseTime, strTime));
};
...
var $author$project$Video$decodeSubtitles = function (videoId) {
	return A2(
		$elm$json$Json$Decode$map,
		$elm$core$List$map(
			function (_v0) {
				var time = _v0.a;
				var text = _v0.b;
				return A3($author$project$Video$Subtitle, videoId, text, time);
			}),
		A2(
			$elm$json$Json$Decode$map,
				...
				$elm$core$List$map(
					$elm$core$Tuple$mapFirst(
						A2(
							...
							$author$project$Video$fromStrTime,
...
var $author$project$Video$decodeVideo = function (videoId) {
	return A5(
		$elm$json$Json$Decode$map4,
		$author$project$Video$Video,
		$elm$json$Json$Decode$succeed(videoId),
		...
		A2(
			...
			A2(
				...
				A2(
					...
					A2(
						$elm$core$Basics$composeR,
						$elm$core$Maybe$map($elm$core$Basics$toFloat),
						$elm$core$Maybe$withDefault(3600))),
				$elm$json$Json$Decode$string)),
		A2(
			$elm$json$Json$Decode$field,
			'subtitles',
			$author$project$Video$decodeSubtitles(videoId)));
};
...
var $author$project$Main$fetchVideo = function (videoId) {
	return $elm$http$Http$get(
		{
			expect: A2(
				$elm$http$Http$expectJson,
				$author$project$Main$GotVideo,
				$author$project$Video$decodeVideo(videoId)),
			url: $author$project$Main$backendUrlRoot + (videoId + '.json')
		});
};
var $author$project$Main$videoIds = _List_fromArray(
	['bmgZsgcPd_8', 'gx874_psuUk', 'RUkjrZZolSo']);
...
var $author$project$Main$init = function (recordingsJson) {
	return _Utils_Tuple2(
		{
			recordings: A2(
				...
				A2(
					...
					$elm$json$Json$Decode$decodeString($author$project$Main$decodeRecordings),
					A2($elm$json$Json$Decode$decodeValue, $elm$json$Json$Decode$string, recordingsJson))),
			tabId: 0,
			videoId: $elm$core$Maybe$Nothing,
			videoIsPlaying: false,
			videoTime: 0,
			videos: _List_Nil
		},
		$elm$core$Platform$Cmd$batch(
			A2($elm$core$List$map, $author$project$Main$fetchVideo, $author$project$Main$videoIds)));
};
var $author$project$Main$GetVideoTime = function (a) {
	return {$: 'GetVideoTime', a: a};
};
var $author$project$Main$getVideoTime = _Platform_incomingPort('getVideoTime', $elm$json$Json$Decode$float);
var $author$project$Main$subscriptions = function (_v0) {
	return $author$project$Main$getVideoTime($author$project$Main$GetVideoTime);
};
...
var $author$project$Video$getNextSubtitle = F2(
	function (videoTime, subtitles) {
		return A2(
			$elm_community$list_extra$List$Extra$find,
			function (sub) {
				return _Utils_cmp(videoTime, sub.time) < 0;
			},
			subtitles);
	});
...
var $author$project$Video$getPrevSubtitle = F2(
	function (videoTime, subtitles) {
		var timeTolerance = 0.8;
		var _v0 = $elm_community$list_extra$List$Extra$last(
			A2(
				$elm$core$List$filter,
				function (sub) {
					return _Utils_cmp(videoTime, sub.time + timeTolerance) > -1;
				},
				subtitles));
		if (_v0.$ === 'Nothing') {
			return $elm$core$List$head(subtitles);
		} else {
			var subtitle = _v0;
			return subtitle;
		}
	});
var $author$project$Video$getSubtitleAt = F2(
	function (videoTime, subtitles) {
		var _v0 = $elm_community$list_extra$List$Extra$last(
			A2(
				$elm$core$List$filter,
				function (sub) {
					return _Utils_cmp(videoTime, sub.time) > -1;
				},
				subtitles));
		if (_v0.$ === 'Nothing') {
			return $elm$core$List$head(subtitles);
...
var $author$project$Main$getVideo = F2(
	function (videoId, videos) {
		return A2(
...
var $author$project$Main$playRecording = _Platform_outgoingPort(
	'playRecording',
	function ($) {
...
var $author$project$Main$saveRecordings = _Platform_outgoingPort(
	'saveRecordings',
...
var $author$project$Main$startVideo = _Platform_outgoingPort('startVideo', $elm$json$Json$Encode$string);
		...
		switch (msg.$) {
			...
			case 'ListenToVideo':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							tabId: $author$project$Main$listenTabId,
							videoId: $elm$core$Maybe$Just(videoId),
							videoIsPlaying: true
						}),
					_Utils_eq(
						$elm$core$Maybe$Just(videoId),
						model.videoId) ? $author$project$Main$playVideo(_Utils_Tuple0) : $author$project$Main$startVideo(videoId));
			case 'PlayVideo':
				return _Utils_Tuple2(
					_Utils_update(
						model,
			...
			case 'FastForward':
				var maybeNextSubtitleTime = A2(
					...
					A2(
						$elm$core$Maybe$andThen,
						$author$project$Video$getNextSubtitle(model.videoTime),
						A2(
							$elm$core$Maybe$map,
							function ($) {
								return $.subtitles;
							},
							A2($author$project$Main$getVideo, model.videoId, model.videos))));
				if (maybeNextSubtitleTime.$ === 'Nothing') {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				} else {
					var videoTime = maybeNextSubtitleTime.a;
					return _Utils_Tuple2(
						model,
						$author$project$Main$setVideoTime(videoTime));
				}
			case 'FastRewind':
				var maybePrevSubtitleTime = A2(
					$elm$core$Maybe$map,
					function ($) {
						return $.time;
					},
					A2(
						$elm$core$Maybe$andThen,
						$author$project$Video$getPrevSubtitle(model.videoTime),
						A2(
							$elm$core$Maybe$map,
							function ($) {
								return $.subtitles;
							...
							A2($author$project$Main$getVideo, model.videoId, model.videos))));
				if (maybePrevSubtitleTime.$ === 'Nothing') {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				} else {
					var videoTime = maybePrevSubtitleTime.a;
					return _Utils_Tuple2(
						model,
						$author$project$Main$setVideoTime(videoTime));
				}
			...
			case 'SetVideoTime':
				...
				return _Utils_Tuple2(
					...
					$author$project$Main$setVideoTime(videoTime));
			case 'SaveRecording':
				var _v3 = A2($author$project$Main$getVideo, model.videoId, model.videos);
				if (_v3.$ === 'Nothing') {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				} else {
					var video = _v3.a;
					var _v4 = A2($author$project$Video$getSubtitleAt, model.videoTime, video.subtitles);
					if (_v4.$ === 'Nothing') {
						return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
					} else {
						var subtitle = _v4.a;
						var recordings = A2(
							...
							F2(
								function (a, b) {
									return (!_Utils_eq(a.videoId, b.videoId)) ? A2($elm$core$Basics$compare, a.time, b.time) : A2($elm$core$Basics$compare, a.videoId, b.videoId);
								}),
						...
						return _Utils_Tuple2(
							...
							$author$project$Main$saveRecordings(recordings));
					}
				}
			case 'PlayRecording':
				var recording = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{videoIsPlaying: true}),
					$author$project$Main$playRecording(recording));
			case 'DeleteRecording':
				var deletedRecording = msg.a;
				var recordings = A2($elm_community$list_extra$List$Extra$remove, deletedRecording, model.recordings);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{recordings: recordings}),
									...
									function (v) {
										return A2(
											...
											A2(
												...
												$elm$core$Basics$eq(v.id),
												$author$project$Main$videoIds));
									},
									$elm_community$list_extra$List$Extra$unique(
										_Utils_ap(
											model.videos,
...
var $author$project$Main$ListenToVideo = function (a) {
	return {$: 'ListenToVideo', a: a};
};
...
var $author$project$Main$PauseVideo = {$: 'PauseVideo'};
...
var $author$project$Main$labeledSymbol = F2(
...
var $author$project$Main$playButton = F2(
	function (model, attributes) {
		return model.videoIsPlaying ? A2(
			$elm$html$Html$button,
			_Utils_ap(
				attributes,
				_List_fromArray(
					[
						$elm$html$Html$Events$onClick($author$project$Main$PauseVideo)
					])),
			_List_fromArray(
				[
					A2($author$project$Main$labeledSymbol, 'Pause', '||')
				])) : A2(
			$elm$html$Html$button,
			_Utils_ap(
				attributes,
			...
			_List_fromArray(
				[
					A2($author$project$Main$labeledSymbol, 'Play', '▶')
				]));
	});
var $author$project$Main$viewVideoCard = F2(
	function (model, video) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('px-5 py-5 mb-4 shadow shadow-slate-100 mx-auto w-full md:w-3/4 lg:w-1/2')
				]),
			_List_fromArray(
				[
					A2(
								...
								[
									...
									_List_fromArray(
										[
											...
											$author$project$Main$ListenToVideo(video.id)),
											$elm$html$Html$Attributes$class('px-3 h-12 bg-cyan-500 hover:bg-cyan-600')
										]),
									_List_fromArray(
										[
									...
									_Utils_eq(
									model.videoId,
									$elm$core$Maybe$Just(video.id)) ? A2(
									$author$project$Main$playButton,
									model,
									_List_fromArray(
										[
											$elm$html$Html$Attributes$class('bg-cyan-500 hover:bg-cyan-600 w-12 h-12')
										])) : $elm$html$Html$text('')
								]))
						]))
				]));
	});
var $author$project$Main$viewFindTab = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		A2(
			$elm$core$List$map,
			$author$project$Main$viewVideoCard(model),
			model.videos));
};
...
var $author$project$Main$findTabId = 0;
...
var $author$project$Main$formatTime = function (totalSeconds) {
	var seconds = A2(
	...
	var minutes = A2(
		$elm$core$Basics$modBy,
		60,
		($elm$core$Basics$floor(totalSeconds) / 60) | 0);
	var hours = ((($elm$core$Basics$floor(totalSeconds) / 60) | 0) / 60) | 0;
	return A2(
		$elm$core$String$join,
		':',
		A2(
...
var $author$project$Main$viewListenTab = function (model) {
	var _v0 = A2($author$project$Main$getVideo, model.videoId, model.videos);
	if (_v0.$ === 'Nothing') {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('mb-2 text-xl')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text('No video selected')
						])),
					A2(
					$elm$html$Html$button,
					_List_fromArray(
						[
							$elm$html$Html$Events$onClick(
							$author$project$Main$TabClicked($author$project$Main$findTabId)),
							$elm$html$Html$Attributes$class('px-3 h-12 bg-cyan-500 hover:bg-cyan-600')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text('Find a video')
						]))
				]));
	} else {
		var video = _v0.a;
		var currentSubtitle = A2(
			$elm$core$Maybe$withDefault,
			A3($author$project$Video$Subtitle, '', '', -1),
			A2($author$project$Video$getSubtitleAt, model.videoTime, video.subtitles));
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('flex flex-col items-center gap-2 h-full')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('text-xl text-center')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(video.title)
						...
						[
							...
							_List_fromArray(
								[
									...
									$elm$core$String$fromFloat(video.duration)),
									$elm$html$Html$Attributes$step('1'),
									$elm$html$Html$Attributes$value(
									$elm$core$String$fromFloat(model.videoTime)),
									...
									A2(
										...
										A2(
											$elm$core$Basics$composeR,
											$elm$core$Maybe$map($author$project$Main$SetVideoTime),
											$elm$core$Maybe$withDefault(
												$author$project$Main$SetVideoTime(0))))),
									$elm$html$Html$Attributes$class('block w-full md:w-3/4 lg:w-1/2 mx-auto')
								]),
							_List_Nil)
						])),
					A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$Events$onClick($author$project$Main$FastRewind),
									$elm$html$Html$Attributes$class('bg-cyan-500 w-12 h-12 hover:bg-cyan-600')
								]),
							_List_fromArray(
								[
									A2($author$project$Main$labeledSymbol, 'Rewind', '<<')
								])),
							A2(
							$author$project$Main$playButton,
							model,
							...
							_List_fromArray(
								[
									$elm$html$Html$Events$onClick($author$project$Main$FastForward),
									$elm$html$Html$Attributes$class('bg-cyan-500 w-12 h-12 hover:bg-cyan-600')
								]),
							_List_fromArray(
								[
									A2($author$project$Main$labeledSymbol, 'Fast-forward', '>>')
								]))
						])),
					A2(
					$elm$html$Html$div,
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$Events$onClick($author$project$Main$SaveRecording),
									$elm$html$Html$Attributes$class('bg-cyan-500 px-16 h-12 hover:bg-cyan-600')
								]),
							_List_fromArray(
								[
					...
					A2(
						...
						function (subtitle) {
							return A2(
								...
								_List_fromArray(
									[
										$elm$html$Html$text(subtitle.text)
									]));
						},
						video.subtitles))
				]));
...
var $author$project$Main$DeleteRecording = function (a) {
	return {$: 'DeleteRecording', a: a};
};
var $author$project$Main$LoadVideo = function (a) {
	return {$: 'LoadVideo', a: a};
};
var $author$project$Main$PlayRecording = function (a) {
	return {$: 'PlayRecording', a: a};
};
...
var $author$project$Main$viewReviewTab = function (model) {
	return $elm$core$List$isEmpty(model.recordings) ? A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('grid gap-4 w-full md:w-3/4 lg:w-1/2 mx-auto')
			]),
		A2(
			$elm$core$List$map,
			function (recording) {
				return A2(
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('grid gap-2 shadow shadow-white p-5')
						]),
					_List_fromArray(
						[
							A2(
								...
								[
									...
									A2(
										...
										A2(
											...
											A2(
												$author$project$Main$getVideo,
												$elm$core$Maybe$Just(recording.videoId),
												model.videos))))
								])),
							A2(
							$elm$html$Html$div,
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$author$project$Main$formatTime(recording.time))
								])),
							A2(
							$elm$html$Html$div,
							_List_Nil,
							_List_fromArray(
								[
									$elm$html$Html$text(recording.text)
								])),
							A2(
							$elm$html$Html$div,
							_List_fromArray(
								...
								[
									...
									_List_fromArray(
										[
											$elm$html$Html$Events$onClick(
											$author$project$Main$PlayRecording(recording)),
											$elm$html$Html$Attributes$class('px-5 h-12 bg-cyan-500 hover:bg-cyan-600')
										]),
									_List_fromArray(
										[
									...
									_List_fromArray(
										[
											$elm$html$Html$Events$onClick(
											$author$project$Main$LoadVideo(recording.videoId)),
											$elm$html$Html$Attributes$class('px-5 h-12 bg-cyan-500 hover:bg-cyan-600')
										]),
									_List_fromArray(
										[
											$elm$html$Html$text('Load video')
										])),
									A2(
									$elm$html$Html$button,
									_List_fromArray(
										[
											$elm$html$Html$Events$onClick(
											$author$project$Main$DeleteRecording(recording)),
											$elm$html$Html$Attributes$class('px-5 h-12 bg-cyan-500 hover:bg-cyan-600')
										]),
									_List_fromArray(
										[
...
var $author$project$Main$tabs = _List_fromArray(
	[
		{content: $author$project$Main$viewFindTab, icon: 'search', text: 'Find'},
		{content: $author$project$Main$viewListenTab, icon: 'headphones', text: 'Listen'},
		{content: $author$project$Main$viewReviewTab, icon: 'grading', text: 'Review'}
	]);
var $author$project$Main$viewTab = F3(
	function (model, tabId, tab) {
		return A2(
			$elm$html$Html$button,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							...
							_Utils_eq(model.tabId, tabId))
						])),
					$elm$html$Html$Attributes$class('grow h-full flex justify-center items-center cursor-pointer'),
					$elm$html$Html$Events$onClick(
					$author$project$Main$TabClicked(tabId))
				]),
			_List_fromArray(
				[
...
var $author$project$Main$viewTabs = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('h-16 flex text-xl bg-black')
			]),
		A2(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('fixed w-full')
					]),
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('pt-24 px-3')
					]),
				_List_fromArray(
					[
						A2(
						...
						A2(
							$elm$core$Maybe$map,
							function (tab) {
								return tab.content(model);
							},
							A2($elm_community$list_extra$List$Extra$getAt, model.tabId, $author$project$Main$tabs)))
					]))
			]));
};
